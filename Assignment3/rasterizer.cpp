...
static bool insideTriangle(int x, int y, const Vector4f* _v){
    ...
    //Vector3f vec0 = v[0] - v[1];
    //Vector3f vec1 = v[1] - v[2];
    //Vector3f vec2 = v[2] - v[0];

    //Vector3f _vec0 = point - v[1];
    //Vector3f _vec1 = point - v[2];
    //Vector3f _vec2 = point - v[0];
...
void rst::rasterizer::draw(std::vector<Triangle *> &TriangleList) {
    ...
    for (const auto& t:TriangleList)
    {
        ...
        for (auto & vert : v)
        {
            ...
            vert.z() = vert.z() * f1 - f2; // 把z从[-1,1]变换到[-50, -0.1]，并变换成正值
        }
    ...
    for (int x = minX; x <= maxX; x++)
    {
        for (int y = minY; y <= maxY; y++)
        {
            if (insideTriangle(x + 0.5f, y + 0.5f, t.v))
            {
                ...
                if (std::isinf(depth_buf[ind]) || depth_buf[ind] < zp)
                {
                    depth_buf[ind] = zp;

                    // 其实这里的alpha beta gamma是2D空间计算重心得到的，应该重新用空间坐标重新计算，但因为映射到屏幕上的结果类似，因此直接使用。
                    ...
                    // 前三项都是2D空间中的，但因为需要计算空间中点和光源等的向量，因此需要插值得到这个点。
